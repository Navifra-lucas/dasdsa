...
public:
    ...
    /**
     *    @brief Reorders the sub-matrixes s.t. the one at 0 <= i < D goes to 0 <= order[i] < D, i.e. m[*]..[*]_i...[*] swaps with
     * m[*]...[i]...[*]_order[i]...[*].
     *    @param order              A permutation of the matrix indices.
     *    @param p                  The output permuted matrix.
     *    @note Example: transpose a 2D matrix by swapping the access indices - MArray<T,2> m, mt; std::size_t trans_ord[2] = {1, 0};
     ...
     *    @note Example: transpose a 2D matrix by swapping the access indices - MArray<T,2> m, mt; std::size_t trans_ord[2] = {1, 0}; mt =
     * m.permute(trans_ord);
     */
...
public:
     ...
     *    @return The output element at i.
     */
    inline const T& slice(const std::size_t i) const { return *this[i]; }
     ...
     *    @return The output element at i.
     */
    inline T& slice(const std::size_t i) { return *this[i]; }
     ...
     */
    inline std::size_t size() const { return _size[0]; }

    /**
     *    @brief Gives the size of this vector.
     *    @param s                  The output size of this vector.
     */
    inline void size(std::size_t s[1]) const { s[0] = _size[0]; }

    /**
     *    @brief Gives the total size (number of elements in memory) of this vector.
     *    @return The total number of elements in memory.
     */
    inline std::size_t totalSize() const { return size(); }
...
/// The DistanceMap class provides static method to compute a distance field over any multi-dimensional regularly sampled function.
/// The dimension is fixed at compile time.
/// It is also possible to compute the index of the nearest minimum of each sample.
class DistanceMap {
public:
    /**
     *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D gets the distance from the local minima of
     * f.
     *    @param f              A DIM-dimensional, regularly sampled function.
     *    @param D              The resulting distance field of f.
     *    @param squared        Compute squared distances (L2)^2 - avoiding to compute square roots - (true) or keep them normal (false -
     * default).
    ...
    /**
     *    @brief Compute the L2-norm distance field D of a DIM-dimensional sampled function f. D gets the distance from the local minima of
     * f. Compute also the (index of the) nearest local minimum of each sample.
     *    @param f              A DIM-dimensional, regularly sampled function.
     *    @param D              The resulting distance field of f.
    ...
    inline static void distanceTransformL2(
        const MMArray<Scalar, DIM>& f, MMArray<Scalar, DIM>& D, MMArray<std::size_t, DIM>& I, const bool squared = false)
    {
        std::size_t size[DIM];
        f.size(size);
        if (&D != &f)
    ...
    template <typename Scalar = float>
    inline static void distanceTransformL2(
        const MMArray<Scalar, 1>& f, MMArray<Scalar, 1>& D, MMArray<std::size_t, 1>& I, const bool squared = false)
    {
        std::size_t size[1];
        f.size(size);
        if (&D != &f)
...
private:
    ...
    /**
     *    @brief The actual distance field computation as in the "Distance Transforms of Sampled Functions" paper, performed in a
     * mono-dimensional function.
     *    @param f              A 1-dimensional, regularly sampled function.
     *    @param D              The resulting distance field of f.
     */
    template <typename Scalar = float>
    ...
    inline static void distanceL2(
        const MArray<Scalar, DIM>& f, MArray<Scalar, DIM>& D, const MArray<std::size_t, DIM>& Ipre, MArray<std::size_t, DIM>& Ipost)
    {
        MArray<Scalar, DIM - 1> f_q, D_q;
    ...
    template <typename Scalar = float>
    inline static void distanceL2(
        const MArray<Scalar, 1>& f, MArray<Scalar, 1>& D, const MArray<std::size_t, 1>& Ipre, MArray<std::size_t, 1>& Ipost)
    {
        if (f.size() == 0 || f.size() > D.size())
            return;
        if (f.size() == 1) {
