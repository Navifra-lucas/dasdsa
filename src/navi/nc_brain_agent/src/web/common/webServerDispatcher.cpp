...
std::istream* WebServerDispatcher::findResource(
    const std::string& base, const std::string& res, const std::string& index, std::string& mediaType, std::string& resolvedPath,
    bool canCache) const
{
...
std::istream* WebServerDispatcher::getCachedResource(const std::string& path, bool canCache) const
{
    if (_cacheResources && canCache) {
        ...
        NLOG(debug) << Poco::format("Cannot cache: %s", path);
        return 0;
    }
}
...
void WebServerDispatcher::logRequest(const Poco::Net::HTTPServerRequest& request, const Poco::Net::HTTPServerResponse& response)
{
    ...
    Poco::Message message("HTTP", reqText, Poco::Message::PRIO_INFORMATION);
...
void WebServerDispatcher::sendResource(
    Poco::Net::HTTPServerRequest& request, const std::string& path, const std::string& vpath, const std::string& resPath,
    const std::string& resBase, const std::string& index, bool canCache)
{
...
bool WebServerDispatcher::handleCORS(
    const Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response, const VirtualPath& vPath) const
{
...
bool WebServerDispatcher::enableCORS(
    const Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPServerResponse& response, const VirtualPath& vPath) const
{
    std::string allowOrigin = vPath.cors.allowOrigin;
    if (allowOrigin.empty())
        allowOrigin = _corsAllowedOrigin;
...
void WebServerDispatcher::sendResponse(
...
void WebServerDispatcher::sendHTMLResponse(
    Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPResponse::HTTPStatus status, const std::string& message)
{
...
void WebServerDispatcher::sendJSONResponse(
    Poco::Net::HTTPServerRequest& request, Poco::Net::HTTPResponse::HTTPStatus status, const std::string& message)
{
    ...
    std::string json(Poco::format(
        "{ \"error\": \"%s\", \"detail\": \"%s\", \"code\": %d }", request.response().getReasonForStatus(status), jsonize(message),
