...
#include <nc_brain_agent/action/nc_action_log_download.h>

using namespace NaviFra;

NcActionLogDownload::NcActionLogDownload()
{
}
...
void NcActionLogDownload::implonAction(std::string source, Poco::JSON::Object::Ptr obj)
{
    NLOG(info) << "NcActionLogDownload::implonAction";
    ...
    if (data->has("key") && data->has("log_id") && data->has("type")) {
        ...
        playBackCMD(command);
        sendResponseSuccess(source, obj->get("uuid").convert<std::string>(), "success", "");
    }
    else {
...
std::string NcActionLogDownload::implName()
{
    return "NcActionLogDownload";
}
