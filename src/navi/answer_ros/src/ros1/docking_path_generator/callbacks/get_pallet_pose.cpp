...
void DockingPathGeneratorRos1::GetPalletPose(
    const geometry_msgs::PoseStamped::ConstPtr msg)
{
    ...
    LOG_TRACE(
        "[GetPalletPose] header: {},  pallet pose (x,y,z,yaw): {:.3f} [m], {:.3f} [m], {:.3f} [m], {:.3f} [deg]",
        msg->header.frame_id.c_str(),
        msg->pose.position.x, msg->pose.position.y,
        msg->pose.position.z, yaw_rad * CM::Rad2Deg);

    int n_mission_type = DockingInfo::CMD::NONE;
    {
        std::lock_guard<std::mutex> lock(mtx_mission_);
        n_mission_type = n_mission_type_;
    }
    switch (n_mission_type)
    {
        ...
        case DockingInfo::CMD::WINGBODY:
            if (o_mission_ptr_)
            {
                o_mission_ptr_->SetObjectPose(o_obj_pose);
            }
            else
            {
