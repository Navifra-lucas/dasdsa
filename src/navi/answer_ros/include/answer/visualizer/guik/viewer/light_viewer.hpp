...
class LightViewer
    : public guik::Application
    , public guik::LightViewerContext {
public:
    LightViewer();
    virtual ~LightViewer();
    ...
    static LightViewer *instance(
        ...
        bool background = false, const std::string &title = "screen");
    static void destroy();
    static bool running();

    bool spin_until_click();
    bool toggle_spin_once();
    virtual void register_ui_callback(
        const std::string &name,
        const std::function<void()> &callback = 0) override;
    ...
    void update_image(
        const std::string &name, const std::shared_ptr<glk::Texture> &image,
        double scale = -1.0, int order = -1);

    std::shared_ptr<LightViewerContext> sub_viewer(
        const std::string &context_name,
        const Eigen::Vector2i &canvas_size = Eigen::Vector2i(-1, -1));

    void show_sub_viewers();  // Set all sub viewer windows to be opened
    std::shared_ptr<LightViewerContext> find_sub_viewer(
        const std::string
            &context_name);  // Returns nullptr if sub viewer does not exist
    bool remove_sub_viewer(
        const std::string
            &context_name);  // Returns false if sub viwewer does not exist

    // Plotting methods
    void clear_plots(bool clear_settings = true);
    void remove_plot(
        const std::string &plot_name, const std::string &label = "");
    void setup_plot(
        const std::string &plot_name, int width, int height, int plot_flags = 0,
        int x_flags = 0, int y_flags = 0, int order = -1);
    ...
    void update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::deque<double> &ys, int line_flags = 0,
        size_t max_num_data = 8192 * 12);
    void update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &ys, int line_flags = 0,
        size_t max_num_data = 8192 * 12);
    void update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &xs, const std::vector<double> &ys,
        int line_flags = 0, size_t max_num_data = 8192 * 12);
    void update_plot_scatter(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &ys, int scatter_flags = 0);
    void update_plot_scatter(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &xs, const std::vector<double> &ys,
        int scatter_flags = 0);
    void update_plot_stairs(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &ys, int stairs_flags = 0);
    void update_plot_stairs(
        const std::string &plot_name, const std::string &label,
        const std::vector<double> &xs, const std::vector<double> &ys,
        int stairs_flags = 0);
    ...
    template <typename T>
    auto update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &ys, int line_flags = 0,
        size_t max_num_data = 8192 * 12)
        -> std::enable_if_t<std::is_arithmetic_v<T>, void>;
    template <typename T1, typename T2>
    void update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::vector<T1> &xs, const std::vector<T2> &ys,
        int line_flags = 0, size_t max_num_data = 8192 * 12);
    ...
    auto update_plot_line(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &data, const Func &transform, int line_flags = 0,
        size_t max_num_data = 8192 * 12)
        -> std::enable_if_t<
            !std::is_arithmetic_v<decltype(transform(data[0]))>, void>
    {
    ...
    auto update_plot_scatter(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &ys, int scatter_flags = 0)
        -> std::enable_if_t<std::is_arithmetic_v<T>, void>;
    template <typename T1, typename T2>
    void update_plot_scatter(
        const std::string &plot_name, const std::string &label,
    ...
    auto update_plot_scatter(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &data, const Func &transform,
        int scatter_flags = 0, size_t max_num_data = 8192 * 12)
        -> std::enable_if_t<
            !std::is_arithmetic_v<decltype(transform(data[0]))>, void>
    {
        std::vector<double> xs(data.size());
    ...
    void update_plot_stairs(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &ys, int stairs_flags = 0);
    template <typename T1, typename T2>
    void update_plot_stairs(
        const std::string &plot_name, const std::string &label,
        const std::vector<T1> &xs, const std::vector<T2> &ys,
    ...
    void update_plot_histogram(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &xs, int bins = -2,
        const Eigen::Vector2d &range = Eigen::Vector2d(0.0, 0.0),
        int histogram_flags = 0);
    template <typename T1, typename T2>
    void update_plot_histogram(
        const std::string &plot_name, const std::string &label,
        const std::vector<T1> &xs, const std::vector<T2> &ys, int x_bins = -2,
        int y_bins = -2,
        const Eigen::Vector2d &x_range = Eigen::Vector2d(0.0, 0.0),
        const Eigen::Vector2d &y_range = Eigen::Vector2d(0.0, 0.0),
        int histogram_flags = 0);
    ...
    void update_plot_histogram(
        const std::string &plot_name, const std::string &label,
        const std::vector<Eigen::Matrix<T, D, 1>, Alloc> &data, int x_bins = -2,
        int y_bins = -2,
        const Eigen::Vector2d &x_range = Eigen::Vector2d(0.0, 0.0),
        const Eigen::Vector2d &y_range = Eigen::Vector2d(0.0, 0.0),
        int histogram_flags = 0);
    ...
    void update_plot_histogram(
        const std::string &plot_name, const std::string &label,
        const std::vector<T> &data, const Func &transform, int x_bins = -2,
        int y_bins = -2,
        const Eigen::Vector2d &x_range = Eigen::Vector2d(0.0, 0.0),
        const Eigen::Vector2d &y_range = Eigen::Vector2d(0.0, 0.0),
        int histogram_flags = 0);
    ...
    void set_scatter_style(
        const std::string &plot_name, const std::string &label, int marker = 0,
        float size = -1,
...
private:
    ...
    virtual bool init(
        const Eigen::Vector2i &size, const char *glsl_version, bool background,
        const std::string &title) override;
    virtual void framebuffer_size_callback(
        const Eigen::Vector2i &size) override;
...
private:
    ...
    std::unordered_map<
        std::string, std::tuple<double, std::shared_ptr<glk::Texture>, int>>
        images;
    std::vector<std::shared_ptr<glk::Texture>> images_in_rendering;

    std::unordered_map<int, Eigen::Matrix<double, 6, 2>>
        plot_linked_axis_limits;
    std::unordered_map<std::string, PlotSetting> plot_settings;
    std::unordered_map<std::string, int> plot_group_orders;
    std::unordered_map<
        std::string,
        std::vector<std::pair<PlotStyleConstPtr, PlotDataConstPtr>>>
        plot_data;

    std::unordered_map<std::string, std::shared_ptr<LightViewerContext>>
        sub_contexts;
...
template <typename T>
auto LightViewer::update_plot_line(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &ys, int line_flags, size_t max_num_data)
    -> std::enable_if_t<std::is_arithmetic_v<T>, void>
{
...
void LightViewer::update_plot_line(
    const std::string &plot_name, const std::string &label,
    const std::vector<T1> &xs, const std::vector<T2> &ys, int line_flags,
    size_t max_num_data)
{
...
void LightViewer::update_plot_line(
    const std::string &plot_name, const std::string &label,
    const std::vector<Eigen::Matrix<T, D, 1>, Alloc> &data, int line_flags,
    size_t max_num_data)
{
    std::vector<double> xs(data.size());
    std::vector<double> ys(data.size());
    std::transform(
        data.begin(), data.end(), xs.begin(),
        [](const Eigen::Matrix<T, D, 1> &v) { return v[0]; });
    std::transform(
        data.begin(), data.end(), ys.begin(),
        [](const Eigen::Matrix<T, D, 1> &v) { return v[1]; });
    update_plot_line(plot_name, label, xs, ys, line_flags, max_num_data);
}

template <typename T>
auto LightViewer::update_plot_scatter(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &ys, int scatter_flags)
    -> std::enable_if_t<std::is_arithmetic_v<T>, void>
{
    std::vector<double> ys_(ys.size());
    std::copy(ys.begin(), ys.end(), ys_.begin());
...
void LightViewer::update_plot_scatter(
    const std::string &plot_name, const std::string &label,
    const std::vector<T1> &xs, const std::vector<T2> &ys, int scatter_flags)
{
    std::vector<double> xs_(xs.size());
    std::vector<double> ys_(ys.size());
    std::copy(xs.begin(), xs.end(), xs_.begin());
...
void LightViewer::update_plot_scatter(
    const std::string &plot_name, const std::string &label,
    const std::vector<Eigen::Matrix<T, D, 1>, Alloc> &data, int scatter_flags)
{
    std::vector<double> xs(data.size());
    std::vector<double> ys(data.size());
    std::transform(
...
template <typename T>
void LightViewer::update_plot_stairs(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &ys, int stairs_flags)
{
    std::vector<double> ys_(ys.size());
    std::copy(ys.begin(), ys.end(), ys_.begin());
...
void LightViewer::update_plot_stairs(
    const std::string &plot_name, const std::string &label,
    const std::vector<T1> &xs, const std::vector<T2> &ys, int stairs_flags)
{
    std::vector<double> xs_(xs.size());
    std::vector<double> ys_(ys.size());
    std::copy(xs.begin(), xs.end(), xs_.begin());
...
void LightViewer::update_plot_stairs(
    const std::string &plot_name, const std::string &label,
    const std::vector<Eigen::Matrix<T, D, 1>, Alloc> &data, int stairs_flags)
{
    std::vector<double> xs(data.size());
    std::vector<double> ys(data.size());
    std::transform(
...
void LightViewer::update_plot_stairs(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &data, const Func &transform, int stairs_flags)
{
    std::vector<double> xs(data.size());
    std::vector<double> ys(data.size());
    for (size_t i = 0; i < data.size(); i++) {
...
void LightViewer::update_plot_histogram(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &xs, int bins, const Eigen::Vector2d &range,
    int histogram_flags)
{
    std::vector<double> xs_(xs.size());
    std::copy(xs.begin(), xs.end(), xs_.begin());
...
void LightViewer::update_plot_histogram(
    const std::string &plot_name, const std::string &label,
    const std::vector<T1> &xs, const std::vector<T2> &ys, int x_bins,
    int y_bins, const Eigen::Vector2d &x_range, const Eigen::Vector2d &y_range,
    int histogram_flags)
{
    std::vector<double> xs_(xs.size());
    std::vector<double> ys_(ys.size());
    ...
    update_plot_histogram(
        plot_name, label, xs_, ys_, x_bins, y_bins, x_range, y_range,
        histogram_flags);
}
...
void LightViewer::update_plot_histogram(
    const std::string &plot_name, const std::string &label,
    const std::vector<Eigen::Matrix<T, D, 1>, Alloc> &data, int x_bins,
    int y_bins, const Eigen::Vector2d &x_range, const Eigen::Vector2d &y_range,
    int histogram_flags)
{
    std::vector<double> xs(data.size());
    ...
    std::transform(
        ...
        [](const Eigen::Matrix<T, D, 1> &v) { return v[1]; });
    update_plot_histogram(
        plot_name, label, xs, ys, x_bins, y_bins, x_range, y_range,
        histogram_flags);
}
...
void LightViewer::update_plot_histogram(
    const std::string &plot_name, const std::string &label,
    const std::vector<T> &data, const Func &transform, int x_bins, int y_bins,
    const Eigen::Vector2d &x_range, const Eigen::Vector2d &y_range,
    int histogram_flags)
{
    std::vector<double> xs(data.size());
    std::vector<double> ys(data.size());
