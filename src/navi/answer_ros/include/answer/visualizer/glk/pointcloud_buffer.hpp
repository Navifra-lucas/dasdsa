...
public:
    ...
    void add_normals(
        const std::vector<Eigen::Matrix<float, N, 1>, Allocator> &normals);
    template <int N, typename Allocator>
    void add_normals(
        const std::vector<Eigen::Matrix<double, N, 1>, Allocator> &normals);
    template <int N>
    void add_normals(const Eigen::Matrix<float, N, 1> *normals, int num_points);
    template <int N>
    void add_normals(
        const Eigen::Matrix<double, N, 1> *normals, int num_points);

    template <typename Scalar, typename Allocator>
    void add_color(
        const std::vector<Eigen::Matrix<Scalar, 4, 1>, Allocator> &colors);
    void add_color(const Eigen::Vector4f *colors, int num_points);
    void add_color(const Eigen::Vector4d *colors, int num_points);

    void add_intensity(
        glk::COLORMAP colormap, const std::vector<float> &intensities,
        float scale = 1.0f);
    void add_intensity(
        glk::COLORMAP colormap, const std::vector<double> &intensities,
        float scale = 1.0f);
    void add_intensity(
        glk::COLORMAP colormap, const float *intensities, const int num_points,
        float scale = 1.0f);
    void add_intensity(
        glk::COLORMAP colormap, const double *intensities, const int num_points,
        float scale = 1.0f);

    void add_normals(const float *data, int stride, int num_points);
    void add_color(const float *data, int stride, int num_points);
    void add_intensity(
        glk::COLORMAP colormap, const float *data, int stride, int num_points,
        float scale = 1.0f);
    void add_buffer(
        const std::string &attribute_name, int dim, const float *data,
        int stride, int num_points);

    // Partial attribute update methods (User must ensure that stride and dim
    // are matched with existing attribute)
    void update_buffer_with_indices(
        GLuint buffer, const float *data, int stride,
        const unsigned int *indices, int num_indices);
    void update_buffer_with_indices(
        const std::string &attribute_name, int dim, const float *data,
        int stride, const unsigned int *indices, int num_indices);

    template <typename Scalar, int D, typename Allocator>
    void update_points_with_indices(
        const std::vector<Eigen::Matrix<Scalar, D, 1>, Allocator> &points,
        const std::vector<unsigned int> &indices);
    void update_points_with_indices(
        const Eigen::Vector3f *points, const unsigned int *indices,
        int num_indices);
    void update_points_with_indices(
        const Eigen::Vector4f *points, const unsigned int *indices,
        int num_indices);
    void update_points_with_indices(
        const Eigen::Vector3d *points, const unsigned int *indices,
        int num_indices);
    void update_points_with_indices(
        const Eigen::Vector4d *points, const unsigned int *indices,
        int num_indices);
    void update_points_with_indices(
        const float *data, int stride, const unsigned int *indices,
        int num_indices);

    template <typename Scalar, typename Allocator>
    void update_color_with_indices(
        const std::vector<Eigen::Matrix<Scalar, 4, 1>, Allocator> &colors,
        const std::vector<unsigned int> &indices);
    void update_color_with_indices(
        const Eigen::Vector4f *colors, const unsigned int *indices,
        int num_indices);
    void update_color_with_indices(
        const Eigen::Vector4d *colors, const unsigned int *indices,
        int num_indices);
...
template <int N, typename Allocator>
void PointCloudBuffer::add_normals(
    const std::vector<Eigen::Matrix<float, N, 1>, Allocator> &normals)
{
    ...
    const std::vector<Eigen::Matrix<double, N, 1>, Allocator> &normals)
{
    ...
    std::transform(
        normals.begin(), normals.end(), normals_f.begin(),
        [](const Eigen::Matrix<double, N, 1> &n) {
            return n.template head<3>().template cast<float>();
        });
...
template <int N>
void PointCloudBuffer::add_normals(
    const Eigen::Matrix<float, N, 1> *normals, int num_points)
{
    ...
    const Eigen::Matrix<double, N, 1> *normals, int num_points)
{
    ...
    std::transform(
        ...
        [](const Eigen::Matrix<double, N, 1> &p) {
            return p.template cast<float>();
        });
    add_normals(normals_f);
}

template <typename Scalar, typename Allocator>
void PointCloudBuffer::add_color(
    const std::vector<Eigen::Matrix<Scalar, 4, 1>, Allocator> &colors)
{
...
template <typename Scalar, int D, typename Allocator>
void PointCloudBuffer::update_points_with_indices(
    const std::vector<Eigen::Matrix<Scalar, D, 1>, Allocator> &points,
    const std::vector<unsigned int> &indices)
{
    if (points.size() != indices.size()) {
        std::cerr << "error: points.size() != indices.size() (" << points.size()
                  << " vs " << indices.size() << ")" << std::endl;
    }
    update_points_with_indices(points.data(), indices.data(), indices.size());
}

template <typename Scalar, typename Allocator>
void PointCloudBuffer::update_color_with_indices(
    const std::vector<Eigen::Matrix<Scalar, 4, 1>, Allocator> &colors,
    const std::vector<unsigned int> &indices)
{
    if (colors.size() != indices.size()) {
        std::cerr << "error: colors.size() != indices.size() (" << colors.size()
                  << " vs " << indices.size() << ")" << std::endl;
    }
