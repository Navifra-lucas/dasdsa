...
protected:
    ...
    inline bool is_zenity() const { return flags(flag::has_zenity) || flags(flag::has_matedialog) || flags(flag::has_qarma); }
...
public:
    ...
    void start(std::string const& command)
    {
        ...
        if (!CreateProcessW(nullptr, (LPWSTR)wcommand.c_str(), nullptr, nullptr, FALSE, CREATE_NEW_CONSOLE, nullptr, nullptr, &si, &m_pi))
...
protected:
    ...
    std::string desktop_helper() const
    {
        ...
        return flags(flag::has_zenity)
            ? "zenity"
            : flags(flag::has_matedialog) ? "matedialog" : flags(flag::has_qarma) ? "qarma" : flags(flag::has_kdialog) ? "kdialog" : "echo";
#endif
    }
    ...
    std::string powershell_quote(std::string const& str) const { return "'" + std::regex_replace(str, std::regex("['\"]"), "$&$&") + "'"; }
    ...
    std::string shell_quote(std::string const& str) const { return "'" + std::regex_replace(str, std::regex("'"), "'\\''") + "'"; }
...
protected:
    ...
    file_dialog(
        type in_type, std::string const& title, std::string const& default_path = "", std::vector<std::string> filters = {},
        bool allow_multiselect = false, bool confirm_overwrite = true)
    {
#if _WIN32
        ...
        m_async->start(
            [this, in_type, title, default_path, filter_list, allow_multiselect, confirm_overwrite](int* exit_code) -> std::string {
                (void)exit_code;
...
public:
    notify(std::string const& title, std::string const& message, icon icon = icon::info)
    {
        ...
        // - NIF_INFO    Use a balloon ToolTip instead of a standard ToolTip. The szInfo, uTimeout, szInfoTitle, and dwInfoFlags members are
        // valid.
        // - NIF_GUID    Reserved.
        ...
        if (is_osascript()) {
            ...
            command +=
                " --icon " + get_icon_name(icon) + " --title " + shell_quote(title) + " --passivepopup " + shell_quote(message) + " 5";
        }
...
public:
    open_file(
        std::string const& title, std::string const& default_path = "", std::vector<std::string> filters = {"All Files", "*"},
