...
public:
    ...
    //
    void SetCorrection(const Pose2D &correction)
    {
        this->correction = correction;
    }
    Pose2D GetCorrection() const { return correction; }
    //
...
namespace SLAM2D {
struct ICPResult : public IMatchingResult {
public:
    ICPResult()
        : target_index(0)
        , source_index(0)
    ...
    ICPResult(const bool valid, const float match_ratio, const float rmse)
        : target_index(0)
        , source_index(0)
    {
        SetValid(valid);
...
public:
    ...
    std::string PrintResult()
    {
        ...
        if (GetValid()) {
            return answer_utils::ColorizeString(ss.str(), answer_utils::GREEN);
        }
        else {
...
namespace LOCALIZATION2D {
struct LocalizeResult : public IMatchingResult {
public:
    LocalizeResult()
    {
        f_search_dist = 0.f;
