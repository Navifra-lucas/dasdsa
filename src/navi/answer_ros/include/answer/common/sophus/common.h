...
#undef SOPHUS_COMPILE_TIME_FMT
    ...
    #define SOPHUS_FMT_CSTR(description, ...) fmt::format(SOPHUS_FMT_STRING(description), ##__VA_ARGS__).c_str()

    #define SOPHUS_FMT_STR(description, ...) fmt::format(SOPHUS_FMT_STRING(description), ##__VA_ARGS__)
...
void ensureFailed(char const* function, char const* file, int line, char const* description);
    ...
    #define SOPHUS_ENSURE(expr, description, ...) \
        ((expr) ? ((void)0) : ::Sophus::ensureFailed(SOPHUS_FUNCTION, __FILE__, __LINE__, SOPHUS_FMT_CSTR(description, ##__VA_ARGS__)))
#else
    ...
    #define SOPHUS_DEDAULT_ENSURE_FAILURE_IMPL(function, file, line, description, ...) \
        ...
        #define SOPHUS_ENSURE(expr, ...)                                                                    \
            do {                                                                                            \
                if (!(expr)) {                                                                              \
                    SOPHUS_DEDAULT_ENSURE_FAILURE_IMPL(SOPHUS_FUNCTION, __FILE__, __LINE__, ##__VA_ARGS__); \
                }                                                                                           \
            } while (false)
    #endif
#endif
...
struct IsUniformRandomBitGenerator {
    static const bool value = std::is_unsigned<typename G::result_type>::value && std::is_unsigned<decltype(G::min())>::value &&
