...
void calcW_derivatives(
    Scalar const theta, Scalar const sigma, Scalar& A, Scalar& B, Scalar& C, Scalar& A_dsigma, Scalar& B_dsigma, Scalar& C_dsigma,
    Scalar& A_dtheta, Scalar& B_dtheta)
{
    using std::abs;
    using std::cos;
    using std::exp;
    ...
    if (abs(sigma) < Constants<Scalar>::epsilon()) {
        ...
        if (abs(theta) < Constants<Scalar>::epsilon()) {
            ...
            B_dtheta = -(two * theta - three * sin_theta + theta * cos_theta) / (theta_c * theta);
            A_dsigma = (sin_theta - theta * cos_theta) / theta_c;
            B_dsigma = (half - (cos_theta + theta * sin_theta - one) / theta_sq) / theta_sq;
        }
    }
    else {
        C = (scale - one) / sigma;
        ...
        if (abs(theta) < Constants<Scalar>::epsilon()) {
            ...
            B_dsigma = (scale * (half * sigma_c - (one + half) * sigma_sq + three * sigma - three) + three) / (sigma_c * sigma);
            ...
            A_dtheta =
                (two * (theta_b_one - a_sigma)) / c_sq + (b_sigma - b + a_theta + one) / c_theta + (theta_b_one - a_sigma) / theta_sq_c;
            A_dsigma = (a - b_theta + a_sigma) / c_theta - (two_sigma * (theta - b_theta + a_sigma)) / (theta * c_sq);

            B = (C - (sigma_b_one + a_theta) / (c)) * one / (theta_sq);
            B_dtheta = ((two_theta * (b_sigma - sigma + a_theta)) / c_sq - ((a + b_theta - a_sigma)) / c) / theta_sq -
                (two * ((scale - one) / sigma - (b_sigma - sigma + a_theta) / c)) / theta_c;
            B_dsigma = -((b_sigma + a_theta + b_one) / c + (scale - one) / sigma_sq - (two_sigma * (sigma_b_one + a_theta)) / c_sq -
                         scale / sigma) /
                theta_sq;
        }
    }
...
Matrix<Scalar, N, N> calcWInv(Matrix<Scalar, N, N> const& Omega, Scalar const theta, Scalar const sigma, Scalar const scale)
{
    ...
    if (abs(sigma * sigma) < Constants<Scalar>::epsilon()) {
        ...
        if (abs(theta_sq) < Constants<Scalar>::epsilon()) {
            ...
            b = -scale * (theta * s_sin_theta - theta * sin_theta + sigma * s_cos_theta - scale * sigma + sigma * cos_theta - sigma) /
