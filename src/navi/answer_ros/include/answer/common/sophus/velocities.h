...
Vector3<Scalar> finiteDifferenceRotationalVelocity(
    std::function<SO3<Scalar>(Scalar)> const& foo_R_bar, Scalar t, Scalar h = Constants<Scalar>::epsilon())
{
    // https://en.wikipedia.org/w/index.php?title=Angular_velocity&oldid=791867792#Angular_velocity_tensor
    //
    // W = dR(t)/dt * R^{-1}(t)
    Matrix3<Scalar> dR_dt_in_frame_foo = curveNumDiff([&foo_R_bar](Scalar t0) -> Matrix3<Scalar> { return foo_R_bar(t0).matrix(); }, t, h);
    // velocity tensor
...
template <class Scalar>
Vector3<Scalar> finiteDifferenceRotationalVelocity(
    std::function<SE3<Scalar>(Scalar)> const& foo_T_bar, Scalar t, Scalar h = Constants<Scalar>::epsilon())
{
    return finiteDifferenceRotationalVelocity<Scalar>([&foo_T_bar](Scalar t) -> SO3<Scalar> { return foo_T_bar(t).so3(); }, t, h);
}
