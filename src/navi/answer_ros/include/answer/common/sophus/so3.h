...
public:
    ...
    SOPHUS_FUNC static QuaternionProductType QuaternionProduct(const QuaternionTypeA& a, const QuaternionTypeB& b)
    {
    ...
    SOPHUS_FUNC SO3Product<OtherDerived> operator*(SO3Base<OtherDerived> const& other) const
    {
        ...
        return SO3Product<OtherDerived>(QuaternionProduct<QuaternionProductType>(a, b));
    }
    ...
    ///
    SOPHUS_FUNC Hyperplane operator*(Hyperplane const& p) const { return Hyperplane((*this) * p.normal(), p.offset()); }
    ...
    ///
    SOPHUS_FUNC QuaternionType const& unit_quaternion() const { return static_cast<Derived const*>(this)->unit_quaternion(); }

private:
    /// Mutator of unit_quaternion is private to ensure class invariant. That is
    /// the quaternion must stay close to unit length.
    ///
    SOPHUS_FUNC QuaternionType& unit_quaternion_nonconst() { return static_cast<Derived*>(this)->unit_quaternion_nonconst(); }
};
...
public:
    ...
    {
        static_assert(std::is_same<typename Eigen::QuaternionBase<D>::Scalar, Scalar>::value, "Input must be of same scalar type");
        Base::normalize();
    }
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, DoF> leftJacobian(Tangent const& omega, optional<Scalar> const& theta_o = nullopt)
    {
        using std::cos;
        using std::sin;
        using std::sqrt;
    ...
    SOPHUS_FUNC static Sophus::Matrix<Scalar, DoF, DoF> leftJacobianInverse(Tangent const& omega, optional<Scalar> const& theta_o = nullopt)
    {
        using std::cos;
        using std::sin;
        using std::sqrt;
        ...
        if (theta_sq < Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()) {
            V_inv = Matrix3<Scalar>::Identity() - Scalar(0.5) * Omega + Scalar(1. / 12.) * (Omega * Omega);
        }
        else {
            ...
            V_inv = Matrix3<Scalar>::Identity() - Scalar(0.5) * Omega +
                (Scalar(1) - Scalar(0.5) * theta * cos(half_theta) / sin(half_theta)) / (theta * theta) * (Omega * Omega);
        }
        return V_inv;
    }
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, 3, DoF> Dx_exp_x_times_point_at_0(Point const& point) { return hat(-point); }
    ...
    SOPHUS_FUNC static SO3<Scalar> expAndTheta(Tangent const& omega, Scalar* theta)
    {
        ...
        SOPHUS_ENSURE(
            ...
            "SO3::exp failed! omega: {}, real: {}, img: {}", SOPHUS_FMT_ARG(omega.transpose()), SOPHUS_FMT_ARG(real_factor),
            SOPHUS_FMT_ARG(imag_factor));
        return q;
    }
    ...
    {
        static_assert(
            IsUniformRandomBitGenerator<UniformRandomBitGenerator>::value, "generator must meet the UniformRandomBitGenerator concept");
...
public:
    ...
    /// Accessor of unit quaternion.
    ///
    SOPHUS_FUNC Map<Eigen::Quaternion<Scalar> const, Options> const& unit_quaternion() const { return unit_quaternion_; }
