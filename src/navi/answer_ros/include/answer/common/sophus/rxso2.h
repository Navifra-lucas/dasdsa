...
template <class Scalar_, int Options_>
struct traits<Map<Sophus::RxSO2<Scalar_> const, Options_>> : traits<Sophus::RxSO2<Scalar_, Options_> const> {
    static constexpr int Options = Options_;
    using Scalar = Scalar_;
...
public:
    ...
    {
        typename RxSO2<NewScalarType>::ComplexType c = complex().template cast<NewScalarType>();
    ...
    SOPHUS_FUNC RxSO2Product<OtherDerived> operator*(RxSO2Base<OtherDerived> const& other) const
    {
        ...
        if (squared_scale > Scalar(1.) / (Constants<ResultT>::epsilon() * Constants<ResultT>::epsilon())) {
            /// Saturation to ensure class invariant.
            result_complex.normalize();
            result_complex /= Constants<ResultT>::epsilonPlus();
        }
    ...
    ///
    SOPHUS_FUNC Line operator*(Line const& l) const { return Line((*this) * l.origin(), (*this) * l.direction() / scale()); }
    ...
    SOPHUS_FUNC Hyperplane operator*(Hyperplane const& p) const
    {
        ...
        return Hyperplane((*this) * p.normal() / this_scale, this_scale * p.offset());
    }
    ...
    {
        SOPHUS_ENSURE(
            z.squaredNorm() > Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(), "Scale factor must be greater-equal epsilon.");
        SOPHUS_ENSURE(
            z.squaredNorm() < Scalar(1.) / (Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()),
...
public:
    ...
    ///
    SOPHUS_FUNC explicit RxSO2(Vector2<Scalar> const& z)
        : complex_(z)
    {
        SOPHUS_ENSURE(
            complex_.squaredNorm() >= Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon(),
            "Scale factor must be greater-equal epsilon: {} vs {}", SOPHUS_FMT_ARG(complex_.squaredNorm()),
            SOPHUS_FMT_ARG(Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()));
        SOPHUS_ENSURE(
            complex_.squaredNorm() <= Scalar(1.) / (Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()),
            "Inverse scale factor must be greater-equal epsilon: {} vs {}", SOPHUS_FMT_ARG(Scalar(1.) / complex_.squaredNorm()),
            SOPHUS_FMT_ARG(Constants<Scalar>::epsilon() * Constants<Scalar>::epsilon()));
    }
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, num_parameters, DoF> Dx_exp_x(Tangent const& a)
    {
        using std::cos;
        using std::exp;
        using std::sin;
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, 2, DoF> Dx_exp_x_times_point_at_0(Point const& point)
    {
...
public:
    ...
    SOPHUS_FUNC
    explicit Map(Scalar const* coeffs)
        : complex_(coeffs)
    {
    }
