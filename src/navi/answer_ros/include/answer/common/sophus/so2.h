...
template <class Scalar_, int Options_>
struct traits<Map<Sophus::SO2<Scalar_> const, Options_>> : traits<Sophus::SO2<Scalar_, Options_> const> {
    static constexpr int Options = Options_;
    using Scalar = Scalar_;
...
public:
    ...
    {
        return Matrix<Scalar, DoF, num_parameters>(-unit_complex()[1], unit_complex()[0]);
    }
...
public:
    ...
    ///
    SOPHUS_FUNC SO2()
        : unit_complex_(Scalar(1), Scalar(0))
    {
    }
    ...
    ///
    SOPHUS_FUNC static Sophus::Matrix<Scalar, 2, DoF> Dx_exp_x_times_point_at_0(Point const& point) { return Point(-point.y(), point.x()); }
