...
public:
    ...
    {
        return Sim2<NewScalarType>(rxso2().template cast<NewScalarType>(), translation().template cast<NewScalarType>());
    }
    ...
    SOPHUS_FUNC Transformation matrix() const
    {
        ...
        homogeneous_matrix.row(2) = Matrix<Scalar, 3, 1>(Scalar(0), Scalar(0), Scalar(1));
    ...
    ///
    SOPHUS_FUNC
    typename Eigen::internal::traits<Derived>::RxSO2Type::ComplexType const& complex() const { return rxso2().complex(); }
...
public:
    ...
    SOPHUS_FUNC static Sophus::Matrix<Scalar, num_parameters, DoF> Dx_exp_x(const Tangent& a)
    {
        ...
        Matrix2<Scalar> const Omega2_dtheta = Omega_dtheta * Omega + Omega * Omega_dtheta;
        Matrix2<Scalar> const W = details::calcW<Scalar, 2>(Omega, theta, sigma);
        Vector2<Scalar> const upsilon = a.segment(0, 2);

        Sophus::Matrix<Scalar, num_parameters, DoF> J;
        J.template block<2, 2>(0, 0).setZero();
        J.template block<2, 2>(0, 2) = RxSO2<Scalar>::Dx_exp_x(a.template tail<2>());
        ...
        J.template block<2, 1>(2, 2) = (A_dtheta * Omega + A * Omega_dtheta + B_dtheta * Omega2 + B * Omega2_dtheta) * upsilon;
        J.template block<2, 1>(2, 3) = (A_dsigma * Omega + B_dsigma * Omega2 + C_dsigma * I) * upsilon;
    ...
    SOPHUS_FUNC static Sophus::Matrix<Scalar, 2, DoF> Dx_exp_x_times_point_at_0(Point const& point)
    {
        ...
        J << Sophus::Matrix2<Scalar>::Identity(), Sophus::RxSO2<Scalar>::Dx_exp_x_times_point_at_0(point);
        return J;
    }
...
{
    static_assert(std::is_standard_layout<Sim2>::value, "Assume standard layout for the use of offset of check below.");
    static_assert(
