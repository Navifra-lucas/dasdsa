...
{
    SOPHUS_ENSURE(normal_foo.squaredNorm() > Constants<T>::epsilon(), "{}", SOPHUS_FMT_ARG(normal_foo.transpose()));
...
{
    SOPHUS_ENSURE(
        xDirHint_foo.dot(yDirHint_foo) < Constants<T>::epsilon(), "xDirHint ({}) and yDirHint ({}) must be perpendicular.",
        SOPHUS_FMT_ARG(xDirHint_foo.transpose()), SOPHUS_FMT_ARG(yDirHint_foo.transpose()));
    using std::abs;
    using std::sqrt;
    ...
    SOPHUS_ENSURE(xDirHint_foo_sqr_length > Constants<T>::epsilon(), "{}", SOPHUS_FMT_ARG(xDirHint_foo.transpose()));
    SOPHUS_ENSURE(yDirHint_foo_sqr_length > Constants<T>::epsilon(), "{}", SOPHUS_FMT_ARG(yDirHint_foo.transpose()));
    SOPHUS_ENSURE(normal_foo_sqr_length > Constants<T>::epsilon(), "{}", SOPHUS_FMT_ARG(normal_foo.transpose()));
    ...
    // sanity check
    SOPHUS_ENSURE(
        abs(det - T(1)) < Constants<T>::epsilon(), "Determinant of basis is not 1, but {}. Basis is \n{}\n", SOPHUS_FMT_ARG(det),
        SOPHUS_FMT_ARG(basis_foo));
    return basis_foo;
}
