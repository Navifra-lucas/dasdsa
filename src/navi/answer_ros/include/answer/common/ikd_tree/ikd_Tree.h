...
template <typename PointType, class DataSource>
class KD_TREE {
public:
    using PointVector = vector<PointType, Eigen::aligned_allocator<PointType>>;
    using Ptr = shared_ptr<KD_TREE<PointType, DataSource>>;
    struct KD_TREE_NODE {
        PointType point;
...
private:
    ...
    pthread_mutex_t rebuild_logger_mutex_lock,
        points_deleted_rebuild_mutex_lock;
        ...
        while (!terminated) {
            ...
            if (Rebuild_Ptr != nullptr) {
                ...
                if (int(Rebuild_PCL_Storage.size()) > 0) {
                    BuildTree(
                        &new_root_node, 0, Rebuild_PCL_Storage.size() - 1,
                        Rebuild_PCL_Storage);
                    // Rebuild has been done. Updates the blocked operations
                    // into the new tree
                    pthread_mutex_lock(&working_flag_mutex);
                    ...
                    while (!Rebuild_Logger.empty()) {
                        ...
                        max_queue_size =
                            max(max_queue_size, Rebuild_Logger.size());
                        Rebuild_Logger.pop();
    ...
    void run_operation(KD_TREE_NODE **root, Operation_Logger_Type operation)
    {
        switch (operation.op) {
            case ADD_POINT:
                Add_by_point(
                    root, operation.point, false, (*root)->division_axis);
                break;
            case ADD_BOX:
    ...
    void BuildTree(KD_TREE_NODE **root, int l, int r, PointVector &Storage)
    {
        ...
        *root = new KD_TREE_NODE;
        ...
        float min_value[2] = {INFINITY, INFINITY};
        float max_value[2] = {-INFINITY, -INFINITY};
        float dim_range[2] = {0, 0};
        for (i = l; i <= r; i++) {
        ...
        switch (div_axis) {
            case 0:
                nth_element(
                    begin(Storage) + l, begin(Storage) + mid,
                    begin(Storage) + r + 1, [&](const auto &a, const auto &b) {
                        return a.x() < b.x();
                    });
                break;
            case 1:
        ...
        bool is_downsample)
    {
        ...
        if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr) {
            tmp_counter += Delete_by_range(
                &((*root)->left_son_ptr), boxpoint, allow_rebuild,
                is_downsample);
        }
        else {
            pthread_mutex_lock(&working_flag_mutex);
        ...
        if ((Rebuild_Ptr == nullptr) ||
            (*root)->right_son_ptr != *Rebuild_Ptr) {
            tmp_counter += Delete_by_range(
                &((*root)->right_son_ptr), boxpoint, allow_rebuild,
                is_downsample);
        }
        else {
            pthread_mutex_lock(&working_flag_mutex);
    ...
    void Delete_by_point(
        KD_TREE_NODE **root, PointType point, bool allow_rebuild)
    {
        if ((*root) == nullptr || (*root)->tree_deleted)
            return;
        (*root)->working_flag = true;
        ...
        int father_axis)
    {
        ...
        if (((*root)->division_axis == 0 && point.x() < (*root)->point.x()) ||
            ...
            if ((Rebuild_Ptr == nullptr) ||
                (*root)->left_son_ptr != *Rebuild_Ptr) {
                Add_by_point(
                    &(*root)->left_son_ptr, point, allow_rebuild,
                    (*root)->division_axis);
            }
            else {
                pthread_mutex_lock(&working_flag_mutex);
                Add_by_point(
                    &(*root)->left_son_ptr, point, false,
                    (*root)->division_axis);
                if (rebuild_flag) {
                    pthread_mutex_lock(&rebuild_logger_mutex_lock);
            ...
            if ((Rebuild_Ptr == nullptr) ||
                (*root)->right_son_ptr != *Rebuild_Ptr) {
                Add_by_point(
                    &(*root)->right_son_ptr, point, allow_rebuild,
                    (*root)->division_axis);
            }
            else {
                pthread_mutex_lock(&working_flag_mutex);
                Add_by_point(
                    &(*root)->right_son_ptr, point, false,
                    (*root)->division_axis);
                if (rebuild_flag) {
                    pthread_mutex_lock(&rebuild_logger_mutex_lock);
                    Rebuild_Logger.push(add_log);
        ...
        if (q.size() < k_nearest ||
            ...
            if (dist_left_node <= dist_right_node) {
                ...
                if (q.size() < k_nearest || dist_right_node < q.top().dist) {
                    if (Rebuild_Ptr == nullptr ||
                        *Rebuild_Ptr != root->right_son_ptr) {
                        Search(
                            root->right_son_ptr, k_nearest, point, q, max_dist);
                    }
                    else {
                        pthread_mutex_lock(&search_flag_mutex);
                        while (search_mutex_counter == -1) {
                ...
                if (q.size() < k_nearest || dist_left_node < q.top().dist) {
                    if (Rebuild_Ptr == nullptr ||
                        *Rebuild_Ptr != root->left_son_ptr) {
                        Search(
                            root->left_son_ptr, k_nearest, point, q, max_dist);
                    }
                    else {
                        pthread_mutex_lock(&search_flag_mutex);
                        while (search_mutex_counter == -1) {
        ...
        KD_TREE_NODE *root, PointType point, float radius, PointVector &Storage)
    {
        ...
        range_center.x() =
            (root->node_range_x[0] + root->node_range_x[1]) * 0.5;
        range_center.y() =
            (root->node_range_y[0] + root->node_range_y[1]) * 0.5;
    ...
    void Update(KD_TREE_NODE *root)
    {
        ...
        if (left_son_ptr != nullptr && right_son_ptr != nullptr) {
            root->TreeSize =
                left_son_ptr->TreeSize + right_son_ptr->TreeSize + 1;
            ...
            root->tree_downsample_deleted =
                left_son_ptr->tree_downsample_deleted &
            ...
            if (root->tree_deleted ||
                (!left_son_ptr->tree_deleted && !right_son_ptr->tree_deleted &&
                 !root->point_deleted)) {
                tmp_range_x[0] =
                    min(min(left_son_ptr->node_range_x[0],
                            right_son_ptr->node_range_x[0]),
                        root->point.x());
                tmp_range_x[1] =
                    max(max(left_son_ptr->node_range_x[1],
                            right_son_ptr->node_range_x[1]),
                        root->point.x());
                tmp_range_y[0] =
                    min(min(left_son_ptr->node_range_y[0],
                            right_son_ptr->node_range_y[0]),
                        root->point.y());
                tmp_range_y[1] =
                    max(max(left_son_ptr->node_range_y[1],
                ...
                if (!left_son_ptr->tree_deleted) {
                    tmp_range_x[0] =
                        min(tmp_range_x[0], left_son_ptr->node_range_x[0]);
                    tmp_range_x[1] =
                        max(tmp_range_x[1], left_son_ptr->node_range_x[1]);
                    tmp_range_y[0] =
                        min(tmp_range_y[0], left_son_ptr->node_range_y[0]);
                    tmp_range_y[1] =
                        max(tmp_range_y[1], left_son_ptr->node_range_y[1]);
                }
                if (!right_son_ptr->tree_deleted) {
                    tmp_range_x[0] =
                        min(tmp_range_x[0], right_son_ptr->node_range_x[0]);
                    tmp_range_x[1] =
                        max(tmp_range_x[1], right_son_ptr->node_range_x[1]);
                    tmp_range_y[0] =
                        min(tmp_range_y[0], right_son_ptr->node_range_y[0]);
                    tmp_range_y[1] =
                        max(tmp_range_y[1], right_son_ptr->node_range_y[1]);
                }
                if (!root->point_deleted) {
            ...
            if (root->tree_deleted ||
                (!left_son_ptr->tree_deleted && !root->point_deleted)) {
                tmp_range_x[0] =
                    min(left_son_ptr->node_range_x[0], root->point.x());
                tmp_range_x[1] =
                    max(left_son_ptr->node_range_x[1], root->point.x());
                tmp_range_y[0] =
                    min(left_son_ptr->node_range_y[0], root->point.y());
                tmp_range_y[1] =
                    max(left_son_ptr->node_range_y[1], root->point.y());
            }
            else {
                if (!left_son_ptr->tree_deleted) {
                    tmp_range_x[0] =
            ...
            root->invalid_point_num = right_son_ptr->invalid_point_num +
                (root->point_deleted ? 1 : 0);
            ...
            if (root->tree_deleted ||
                (!right_son_ptr->tree_deleted && !root->point_deleted)) {
                tmp_range_x[0] =
                    min(right_son_ptr->node_range_x[0], root->point.x());
                tmp_range_x[1] =
                    max(right_son_ptr->node_range_x[1], root->point.x());
                tmp_range_y[0] =
                    min(right_son_ptr->node_range_y[0], root->point.y());
                tmp_range_y[1] =
                    max(right_son_ptr->node_range_y[1], root->point.y());
            }
            else {
                if (!right_son_ptr->tree_deleted) {
                    tmp_range_x[0] =
...
public:
    // KD_TREE(float delete_param = 0.5, float balance_param = 0.6, float
    // box_length = 0.2);
    KD_TREE(
        const DataSource &_data_source, float delete_param = 0.5,
        float balance_param = 0.6, float box_length = 0.2)
    ...
    void Build(PointVector point_cloud)
    {
        ...
        BuildTree(
            &STATIC_ROOT_NODE->left_son_ptr, 0, point_cloud.size() - 1,
            point_cloud);
    ...
    void Radius_Search(
        PointType point, const float radius, PointVector &Storage)
    {
        Storage.clear();
    ...
    int Add_Points(PointVector &PointToAdd, bool downsample_on)
    {
        ...
        for (size_t i = 0; i < PointToAdd.size(); i++) {
            if (downsample_switch) {
                ...
                for (int index = 0; index < Downsample_Storage.size();
                     index++) {
                ...
                if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
                    if (Downsample_Storage.size() > 1 ||
                        ...
                        if (Downsample_Storage.size() > 0)
                            Delete_by_range(
                                &Root_Node, Box_of_Point, true, true);
                        Add_by_point(
                    ...
                    if (Downsample_Storage.size() > 1 ||
                        ...
                        if (Downsample_Storage.size() > 0)
                            Delete_by_range(
                                &Root_Node, Box_of_Point, false, true);
                        Add_by_point(
                            &Root_Node, downsample_result, false,
                            Root_Node->division_axis);
                        tmp_counter++;
                        if (rebuild_flag) {
                            pthread_mutex_lock(&rebuild_logger_mutex_lock);
                ...
                std::cout << "PointToAdd Size: " << PointToAdd.size()
                          << std::endl;

                if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
                    std::cout << "i: " << i << std::endl;
                    std::cout << "PointToAdd[i]: " << PointToAdd[i].x() << " "
                              << PointToAdd[i].y() << std::endl;

                    Add_by_point(
                        &Root_Node, PointToAdd[i], true,
                        Root_Node->division_axis);
                }
                else {
                    Operation_Logger_Type operation;
        ...
        std::cout << "Time for adding " << NewPointSize
                  << " points: " << elapsed_seconds.count() << "s\n";

        return tmp_counter;
    };

    int Add_Points_With_Range(
        const size_t start_idx, const size_t end_idx,
        bool downsample_on = false)
    {
        // int NewPointSize = PointToAdd.size();
        int tree_size = size();
        BoxPointType Box_of_Point;
        ...
        auto pcl_cloud = data_source.GetPointCloud();

        std::chrono::time_point<std::chrono::steady_clock> start, end;
        start = std::chrono::steady_clock::now();

        std::cout << "PointToAdd Size: " << pcl_cloud.size() << std::endl;
        std::cout << "downsample_switch: " << downsample_switch << std::endl;

        for (size_t i = 0; i < pcl_cloud.size(); i++) {
            if (downsample_switch) {
                Box_of_Point.vertex_min[0] =
                    floor(pcl_cloud[i].x() / downsample_size) * downsample_size;
                Box_of_Point.vertex_max[0] =
                    Box_of_Point.vertex_min[0] + downsample_size;
                Box_of_Point.vertex_min[1] =
                    floor(pcl_cloud[i].y() / downsample_size) * downsample_size;
                Box_of_Point.vertex_max[1] =
                    Box_of_Point.vertex_min[1] + downsample_size;
                mid_point.x() = Box_of_Point.vertex_min[0] +
                    (Box_of_Point.vertex_max[0] - Box_of_Point.vertex_min[0]) /
                ...
                PointVector().swap(Downsample_Storage);
                Search_by_range(Root_Node, Box_of_Point, Downsample_Storage);
                min_dist = calc_dist(pcl_cloud[i], mid_point);
                downsample_result = pcl_cloud[i];
                for (int index = 0; index < Downsample_Storage.size();
                     index++) {
                    tmp_dist = calc_dist(Downsample_Storage[index], mid_point);
                    if (tmp_dist < min_dist) {
                ...
                if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
                    if (Downsample_Storage.size() > 1 ||
                        same_point(pcl_cloud[i], downsample_result)) {
                        if (Downsample_Storage.size() > 0)
                            Delete_by_range(
                                &Root_Node, Box_of_Point, true, true);
                        Add_by_point(
                ...
                if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
                    Add_by_point(
                        &Root_Node, pcl_cloud[i], true,
                        Root_Node->division_axis);
                }
                else {
                    Operation_Logger_Type operation;
                    operation.point = pcl_cloud[i];
                    operation.op = ADD_POINT;
                    pthread_mutex_lock(&working_flag_mutex);

                    Add_by_point(
                        &Root_Node, pcl_cloud[i], false,
                        Root_Node->division_axis);
                    if (rebuild_flag) {
                        pthread_mutex_lock(&rebuild_logger_mutex_lock);
                        Rebuild_Logger.push(operation);
        ...
        std::chrono::duration<double> elapsed_seconds = end - start;
        std::cout << "Time for adding " << count
                  << " points: " << elapsed_seconds.count() << "s\n";
    ...
    void flatten(
        KD_TREE_NODE *root, PointVector &Storage,
