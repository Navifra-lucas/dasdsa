/**
 * @class mapper
 * @brief SLAM back-end(graph optimization & loop detection thread) using ceres
 * solver & icp.
...
#include "2d_slam/globalmapper/scanmatcher/icp2d.h"
#include "2d_slam/localmapper/localmap/localmap2d.h"
#include "2d_slam/localmapper/localmap/probability_values.h"
...
private:
    ...
    void MergePointCloud(
        Localmap2D *localmap1, Localmap2D *localmap2,
        const Pose2D &localmap1_to_localmap2);
    void AddConstraint(
        ceres::Problem *problem, deque<shared_ptr<Node2D>> poses,
        deque<shared_ptr<Edge2D>> constraint);
    void AddLoopConstraint(
        ceres::Problem *problem, deque<shared_ptr<Node2D>> poses,
        deque<Edge2D> LoopConstraint);
...
public:
    ...
    void AddLocalizationCandidate(
        PointCloud2D localization_candidate, const bool b_initial_pose = false);
    ...
    void GenerateLocalmapProbTable(
        const float hit_probability, const float miss_probability);

    void GenerateHighLocalmapProbTable(
        const float hit_probability, const float miss_probability);
    const std::vector<int8_t> &GetGlobalMap() const
    {
        return global_map_;
    }
    ...
    void SetParam(const slam_parameter_container *slam_param)
    {
        ...
        icp_.SetParameters(
            30, 0.05, 3, slam_param_->f_min_match_ratio,
            slam_param_->f_icp_success_rmse);
    }
    ...
    const std::vector<uint16_t> *GetHitTable()
    {
        return &hit_table_;
    }
    const std::vector<uint16_t> *GetMissTable()
    {
        return &miss_table_;
    }

    const std::vector<uint16_t> *GetHighHitTable()
    {
        return &high_hit_table_;
    }
    const std::vector<uint16_t> *GetHighMissTable()
    {
        return &high_miss_table_;
    }
};
}  // namespace SLAM2D
}  // namespace ANSWER
