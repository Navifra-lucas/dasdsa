...
public:
    ...
    void ProcessGrid(
        ...
        const Eigen::Array2i &offset, const MapLimits &limits_,
        const float f_map_resolution_, const float meter_to_grid);
    ...
    void ComputeCroppedLimits(
        Eigen::Array2i *const offset, CellLimits *const limits) const;
    ...
    bool ApplyLookupTable(
        const Eigen::Array2i &cell_index, const std::vector<uint16_t> &table);

    float GetProbability(const Eigen::Array2i &cell_index) const
    {
        if (!limits().Contains(cell_index))
            return kMinProbability;
        return ProbabilityValues::GetInstance()
    ...
    void ExploreMap(
        const Eigen::Array2i &offset, const int n_start_x, const int n_end_x,
        const int n_start_y, const int n_end_y, const int n_limit_x,
        const int n_limit_y, std::vector<pair<Pose2D, float>> &prob_map,
...
protected:
    ...
    int ToFlatIndex(const Eigen::Array2i &cell_index) const
    {
        ...
        // //NLOG(info) << cell_index.x()<<" "<<cell_index.y();
