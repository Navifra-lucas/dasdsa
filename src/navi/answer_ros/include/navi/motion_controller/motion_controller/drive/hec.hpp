...
class Hec : public MotionController
{
public:
  Hec();
...
private:
  PID<double> o_pid_a_;
  PID<double> o_pid_w_;
  PID<double> o_pid_hec_;

  double CalcHecCurv(const Path& path, const Pose& o_robot_state, int n_closest_idx, const ControlInfo_t& st_control_info, double d_target_spd);
