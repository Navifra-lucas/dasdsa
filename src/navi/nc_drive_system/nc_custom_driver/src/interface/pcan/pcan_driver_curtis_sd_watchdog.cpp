...
void Pcan_driver_curtis_sd::watchdog()
{
    while (running_) {
        /*
        ...
        */
        ...
        if (b_init_&& !b_brake_on_) {
            PCAN_CURTIS_SD_WRITE_2 uo_write2;
            uo_write2.un8_hearbeat = 0x05;  // Heartbeat
            // Pcan_driver::Write(CanID::CAN_RPDO2_ID+st_interface_param_.st_pcan_param.n_FL_traction_pcan_id, uo_write2.byte_space, 1);
...
void Pcan_driver_curtis_sd::Callback(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
    ...
    for (auto [id, data] : motor_data) {
        if(id == MotorID::FL_TRACTION) {
            ...
            std::chrono::duration<double> rpm_duration = std::chrono::system_clock::now() - tp_update;

            d_FL_traction_encoder_rpm_ += data.feedback_velocity * st_driver_param_.st_traction_param.f_FL_traction_encoder_pulse / 60.0f *
                (st_driver_param_.st_wheel_param.f_FL_wheel_diameter_m * M_PI) / st_driver_param_.st_traction_param.f_FL_traction_gear_ratio *
                float(rpm_duration.count());
            data.encoder = d_FL_traction_encoder_rpm_;
        }
        ...
        if(id == MotorID::FL_TRACTION) {
            ...
            encoder_data[id] = 0;
            n_acc = data.encoder - encoder_data[id];
        ...
        if(id == MotorID::FL_TRACTION){
            ...
            motordata.is_error = data.is_error;
            motordata.last_update_time = data.last_update_time;
            motordata.feedback_angle = data.feedback_angle;
            motor_data_info.data.push_back(motordata);
        }
        else if(id == MotorID::FL_ABS_STEER){
            motordata.motor_id = id;
            motordata.status = data.status;
            // motordata.is_enable = true;
            // if(n_check_status == STATUS_OP_ENABLED)
...
void Pcan_driver_curtis_sd::CheckError(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
    ...
    for (auto [id, data] : motor_data) {
        ...
        if (sec.count() > f_time_thres) {
            if (id == MotorID::FL_TRACTION) {
                ...
                motordata.motor_id = id;
                motordata.is_error = true;
                motordata.error_code = core_msgs::NaviAlarm::ERROR_FL_STEERING_MOTOR_FEEDBACK_TIMEOUT;
                motordata.error_msg = "ERROR_FL_STEERING_MOTOR_FEEDBACK_TIMEOUT";
            }
            // else if (id == MotorID::FL_ABS_STEER) {
            //     motordata.motor_id = id;
            //     motordata.is_error = true;
            //     motordata.error_code = core_msgs::NaviAlarm::ERROR_FL_STEERING_ABSOLUTE_ENCODER_TIMEOUT;
            //     motordata.error_msg = "ERROR_FL_STEERING_ABSOLUTE_ENCODER_TIMEOUT";
            // }
        }
        ...
        //         motordata.motor_id = id;
        //         motordata.is_error = true;
        //         motordata.error_code = core_msgs::NaviAlarm::ERROR_FL_STEERING_MOTOR_OVER_CURRENT;
        //         motordata.error_msg = "ERROR_FL_STEERING_MOTOR_OVER_CURRENT";
        //     }
        // }

        static int n_fl_traction_fault_cnt = 0;
        static int n_fl_steer_fault_cnt = 0;
        if (data.is_error == true) {
            if (id == MotorID::FL_TRACTION)
                n_fl_traction_fault_cnt++;
            else if (id == MotorID::FL_STEER)
                n_fl_steer_fault_cnt++;            
        }
        else {
            if (id == MotorID::FL_TRACTION)
                n_fl_traction_fault_cnt = 0;
            else if (id == MotorID::FL_STEER)
                n_fl_steer_fault_cnt = 0;  
        }
        ...
        else if (n_fl_steer_fault_cnt > 10) {
            motordata.motor_id = MotorID::FL_STEER;
            motordata.is_error = true;
            motordata.error_code = core_msgs::NaviAlarm::ERROR_FL_STEERING_MOTOR_FAULT;
            motordata.error_msg = "ERROR_FL_STEERING_MOTOR_FAULT," + oss_error.str();

            std::chrono::duration<double> fl_st_fault_reset = std::chrono::system_clock::now() - current_time[MotorID::FL_STEER];

            if (fl_st_fault_reset.count() > FAULT_RESET_DELAY_SEC) {
                sendControlWord(CONTROL_WORD_FAULT_RESET, MotorID::FL_STEER);  // ControlWord 사용
                current_time[MotorID::FL_STEER] = std::chrono::system_clock::now();
            }
        }
        motor_data_info.data.push_back(motordata);
