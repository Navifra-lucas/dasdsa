...
void Pcan_driver_samyang_dd::watchdog()
{
    while (running_) {
        /*
...
void Pcan_driver_samyang_dd::Callback(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
    ...
    for (auto [id, data] : motor_data) {
        ...
        if(id == MotorID::FL_TRACTION){
            ...
            motordata.acc_encoder = data.acc_encoder;
...
void Pcan_driver_samyang_dd::CheckError(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
    ...
    for (auto [id, data] : motor_data) {
        ...
        if (n_fl_traction_fault_cnt > 10) {
            ...
            if (fl_fault_reset.count() > FAULT_RESET_DELAY_SEC) {
                sendControlWord(CONTROL_WORD_FAULT_RESET, st_interface_param_.st_pcan_param.n_FL_traction_pcan_id);
            ...
            if (rr_fault_reset.count() > FAULT_RESET_DELAY_SEC) {
                sendControlWord(CONTROL_WORD_FAULT_RESET, st_interface_param_.st_pcan_param.n_RR_traction_pcan_id);  // ControlWord 사용
