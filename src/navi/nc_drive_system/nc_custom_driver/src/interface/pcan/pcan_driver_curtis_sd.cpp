...
void Pcan_driver_curtis_sd::Stop()
{
    LOG_INFO("Stop");

    PCAN_CURTIS_SD_WRITE_1 uo_fl_write_1;
    ...
    Pcan_driver::Write(
        CanID::CAN_RPDO1_ID + st_interface_param_.st_pcan_param.n_FL_traction_pcan_id, uo_fl_write_1.byte_space, 8);
    std::this_thread::sleep_for(std::chrono::milliseconds(10));
}

void Pcan_driver_curtis_sd::Initialize()
{
    b_init_ = false;
    Pcan_driver::RegisteCallbackFunc("PcanCallback", std::bind(&Pcan_driver_curtis_sd::PcanCallback, this, std::placeholders::_1));
    TPCANMsg Message;
    Message.ID = 0;
    Message.LEN = 2;
    Message.DATA[0] = NMTCommand::OPERATIONAL;
    Message.DATA[1] = st_interface_param_.st_pcan_param.n_FL_abssteer_pcan_id; // 조향 절댓값 엔코더 받기위한 명령
...
void Pcan_driver_curtis_sd::ControlOff(bool data)
{
   // b_control_off_ = data;
   if (data) {
...
void Pcan_driver_curtis_sd::ReInitializeCheck()
{}

void Pcan_driver_curtis_sd::MotorDriverInit(string& str_data)
{
    ...
    try{
        ...
        nmt_response_manager_.waitForResponse(
            CanID::CAN_NMT_STATUS_ID + st_interface_param_.st_pcan_param.n_FL_abssteer_pcan_id, 10000);
    }
...
void Pcan_driver_curtis_sd::MotorErrorReset(std::string& str_data)
{
...
void Pcan_driver_curtis_sd::EncoderZero(string& str_data)
{
    LOG_INFO("SetEncoderZero");
    Pcan_driver::Write(
        CanID::CAN_SDO_REQUEST_ID + (uint8_t)st_interface_param_.st_pcan_param.n_FL_abssteer_pcan_id, 0x22, 0x03, 0x60, 0x00, 0x00, 0x00, 0x00,
        0x00);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    Pcan_driver::Write(
        CanID::CAN_SDO_REQUEST_ID + (uint8_t)st_interface_param_.st_pcan_param.n_FL_abssteer_pcan_id, 0x22, 0x10, 0x10, 0x01, 0x73, 0x61, 0x76,
        0x65);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

void Pcan_driver_curtis_sd::Write(const Wheel_Cmd_t& st_cmd)
{
    if (!b_init_)
        return;

    PCAN_CURTIS_SD_WRITE_1 uo_write1;
    ...
    // Pcan_driver::Write(
    //     CanID::CAN_RPDO1_ID + st_interface_param_.st_pcan_param.n_FL_traction_pcan_id, uo_write1.byte_space, 8);
    Pcan_driver::Write(0x223, uo_write1.byte_space, 8);
...
void Pcan_driver_curtis_sd::SetMotorIFGain(string& str_data)
{
    LOG_INFO("SET SetMotorIFGain");
}

void Pcan_driver_curtis_sd::SetMotorGain(string& str_data)
{
    ...
    try
    {
        ...
        PCAN_CURTIS_SD_SDO_READ uo_sdo_response;
...
void Pcan_driver_curtis_sd::PcanCallback(const boost::any& any_type_var)
{
    ...
    // NLOG(info)<<"Recv Can ID 0x"<<std::hex<<o_canmsg.ID;
    if (o_canmsg.ID == 0x250) {
        PCAN_CURTIS_SD_READ_1 uo_traction;
    ...
    if (o_canmsg.ID == 0x251) {
        PCAN_CURTIS_SD_READ_2 uo_steer;
    ...
    // abs steer TPDO3
    if (o_canmsg.ID == CanID::CAN_TPDO1_ID + st_interface_param_.st_pcan_param.n_FL_abssteer_pcan_id) {
        PCAN_CURTIS_SD_READ_3 uo_fl_tpdo3;
    ...
    if(o_canmsg.ID == 0x350) {
        std::stringstream ss;
        for(int i = 0; i < o_canmsg.LEN; i++) {
            ss << "0x" << std::setfill('0') << std::setw(2) << std::hex << (int)o_canmsg.DATA[i] << " ";
        }
        NLOG(info)<<"FORK LIFT Feedback Data : " << ss.str();
    }
}

void Pcan_driver_curtis_sd::sendControlWord(ControlWord command, int motor_id)
{
    // // CAN 프레임 생성
    // PCAN_CURTIS_SD_WRITE_2 cmd;
    // int n_id = 0;
    // cmd.control_word = static_cast<uint16_t>(command);  // ControlWord 값을 설정
    // // cmd.n32_target_velocity = 0;
    // // CAN 메시지 전송
    // if(motor_id == MotorID::FL_TRACTION)
    //     n_id = st_interface_param_.st_pcan_param.n_FL_traction_pcan_id;
    // else if(motor_id == MotorID::FL_STEER)
    //     n_id = st_interface_param_.st_pcan_param.n_FL_steer_pcan_id;
    // Pcan_driver::Write(CanID::CAN_RPDO1_ID + n_id, cmd.byte_space, 3);  // CanID::CAN_RPDO1_ID + motor_id: 모터 ID에 맞는 CAN ID
    // std::this_thread::sleep_for(std::chrono::microseconds(100));
    // LOG_INFO("ControlWord 0x%x sent to Motor %d", cmd.control_word, motor_id);
}

void Pcan_driver_curtis_sd::MotorSwitchOn(int motor_id, int n_type)
{
...
motor_msgs::MotorData Pcan_driver_curtis_sd::getMotorData(int motor_id)
{
    std::lock_guard<std::mutex> lock(data_mutex_);  // 멀티스레드 안전
    return motor_data_[motor_id];
}
// 특정 모터 데이터를 설정하는 setter
void Pcan_driver_curtis_sd::SetMotorData(int motor_id, const motor_msgs::MotorData& data)
{
    std::lock_guard<std::mutex> lock(data_mutex_);  // 멀티스레드 안전
    motor_data_[motor_id] = data;
}

void Pcan_driver_curtis_sd::UpdateMotorData(int motor_id, const motor_msgs::MotorData& update_data, int pdo_type)
{
    motor_msgs::MotorData data = getMotorData(motor_id);  // 해당 모터의 데이터를 가져옴

    data.motor_id = motor_id;
    auto now = std::chrono::steady_clock::now();
    if(pdo_type != PDO_ID::RPDO1){
...
void Pcan_driver_curtis_sd::ProcessNmtMessage(int motor_id, const TPCANMsg& o_canmsg)
{
...
void Pcan_driver_curtis_sd::ResetMotorError(int motor_id)
{
    // LOG_INFO("Resetting error for Motor %d", motor_id);
    // int n_id = 0;
    // // 에러 리셋 명령 전송
    // PCAN_CURTIS_SD_WRITE_2 reset_cmd;
    ...
    // else if(motor_id == MotorID::FL_STEER)
    //     n_id = st_interface_param_.st_pcan_param.n_FL_steer_pcan_id;
    // Pcan_driver::Write(CanID::CAN_RPDO1_ID + motor_id, reset_cmd.byte_space, 3);
    TPCANMsg Message;
    Message.ID = 0;
    Message.LEN = 2;
...
bool Pcan_driver_curtis_sd::IsInitializing()
{
    return !b_init_;
}

PCAN_CURTIS_SD_SDO_READ Pcan_driver_curtis_sd::SendSDOMsg(int node_id, int command, int index, int sub_index, int data)
{
    PCAN_CURTIS_SD_SDO_WRITE o_sdo_write;
    ...
    try {
        ...
        PCAN_CURTIS_SD_SDO_READ uo_sdo_read;
...
std::map<int16_t, motor_msgs::MotorData> Pcan_driver_curtis_sd::GetMotorData()
{
    std::lock_guard<std::mutex> lock(data_mutex_);  // 멀티스레드 안전
    return motor_data_;
}