...
void Can_driver_curtis_sd::watchdog()
{
    while (running_) {
        /*
        ...
        */
        ...
        if (b_init_&& !b_brake_on_) {
            CAN_CURTIS_SD_WRITE_2 uo_write2;
            uo_write2.un8_hearbeat = 0x05;  // Heartbeat
            Can_driver::Write(CanID::CAN_RPDO2_ID+st_interface_param_.st_can_param.n_FL_traction_can_id, uo_write2.byte_space, 1);
...
void Can_driver_curtis_sd::Callback(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
...
void Can_driver_curtis_sd::CheckError(std::map<int16_t, motor_msgs::MotorData> motor_data)
{
