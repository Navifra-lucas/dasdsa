...
DockingReachedCondition::DockingReachedCondition(const std::string& name, const BT::NodeConfiguration& config)
...
BT::NodeStatus DockingReachedCondition::tick()
{
    std::string docking_reached;
    std_msgs::String mode;
    if (getInput("docking_reached", docking_reached)) {
        if (docking_reached == "true") {
            config().blackboard->set("docking_reached", "false");
            Task current_task;
            NLOG(info) << docking_reached;
            if (getInput("current_task", current_task)) {
                ...
                // if (current_task.docking_type() == "aruco charge") {
                //     std_msgs::String data;
                //     data.data = "charge_start";
                //     send_startcharge_.publish(data);
                // }
            }
            updateTask();
            return BT::NodeStatus::SUCCESS;
        }
        else if (docking_reached == "error") {
            Task current_task;
            if (getInput("current_task", current_task)) {
                core_msgs::TaskAlarm alarm;
...
BT::PortsList DockingReachedCondition::providedPorts()
{
    return {
        BT::InputPort<std::string>("docking_reached"), BT::InputPort<Task>("current_task"), BT::InputPort<BT::SharedQueue<Task>>("tasks"),
        BT::OutputPort<Task>("current_task"), BT::OutputPort<BT::SharedQueue<Task>>("tasks")};
}

void DockingReachedCondition::updateTask()
{
    try {
