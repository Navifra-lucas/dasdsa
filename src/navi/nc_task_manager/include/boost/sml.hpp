...
template <
    class... Ts, class... T1s, class... T2s, class... T3s, class... T4s, class... T5s, class... T6s, class... T7s, class... T8s,
    class... T9s, class... T10s, class... T11s, class... T12s, class... T13s, class... T14s, class... T15s, class... T16s, class... Us>
struct join<
    type_list<Ts...>, type_list<T1s...>, type_list<T2s...>, type_list<T3s...>, type_list<T4s...>, type_list<T5s...>, type_list<T6s...>,
    ...
    : join<
          type_list<
              Ts..., T1s..., T2s..., T3s..., T4s..., T5s..., T6s..., T7s..., T8s..., T9s..., T10s..., T11s..., T12s..., T13s..., T14s...,
              T15s..., T16s...>,
          Us...> {
};
...
struct unique_impl<type<Rs...>, T, Ts...>
    : conditional_t<is_base_of<type<T>, inherit<type<Rs>...>>::value, unique_impl<type<Rs...>, Ts...>, unique_impl<type<Rs..., T>, Ts...>> {
};
template <class... Rs>
...
struct tuple_type {
    constexpr explicit tuple_type(const T& object)
        : value(object)
    {
    }
    T value;
...
struct pool_type_impl : pool_type_base {
    constexpr explicit pool_type_impl(T object)
        : value{object}
    {
    }
    template <class TObject>
    constexpr pool_type_impl(init i, TObject object)
        : value{i, object}
    {
    }
    T value{};
...
template <class T>
struct pool_type_impl<T&, aux::enable_if_t<aux::is_constructible<T>::value && aux::is_constructible<T, const T&>::value>> : pool_type_base {
    constexpr explicit pool_type_impl(T& value)
        : value{value}
    {
    }
    template <class TObject>
...
struct pool : pool_type<Ts>... {
    ...
    template <class... TArgs>
    constexpr pool(const pool<TArgs...>& p)
        : pool_type<Ts>(init{}, p)...
    {
    }
};
...
struct zero_wrapper : TExpr {
    using type = TExpr;
    constexpr explicit zero_wrapper(const TExpr& expr)
        : TExpr(expr)
    {
    }
...
struct zero_wrapper<R (TBase::*)(TArgs...) const noexcept, T> {
    constexpr explicit zero_wrapper(R (TBase::*ptr)(TArgs...) const noexcept)
        : ptr{ptr}
    {
    }
    constexpr auto operator()(TBase& self, TArgs... args) { return (self.*ptr)(args...); }
...
const char* get_type_name()
{
    ...
    #if defined(__cpp_nontype_template_parameter_class) || defined(__cpp_nontype_template_args) && __cpp_nontype_template_args >= 201911L
template <auto N>
struct fixed_string {
...
public:
    ...
    constexpr explicit queue_event_call(const call_t& call)
        : call{call}
    {
    }
    call_t call{};
...
struct on_entry
    ...
    constexpr explicit on_entry(const TEvent& event = {})
        : event_(event)
    {
    }
    const TEvent& event_;
...
struct on_exit
    ...
    constexpr explicit on_exit(const TEvent& event = {})
        : event_(event)
    {
    }
    const TEvent& event_;
...
using get_history_states =
    aux::join_t<typename aux::conditional<!Ts::history && Ts::initial, aux::type_list<typename Ts::src_state>, aux::type_list<>>::type...>;
template <class>
...
template <class... Ts>
using get_non_empty_t = aux::join_t<typename aux::conditional<aux::is_empty<Ts>::value, aux::type_list<>, aux::type_list<Ts>>::type...>;
template <class... Ts>
...
struct transitions_sub<sm<TSM>, T, Ts...> {
    ...
    constexpr static bool execute_impl(
        const back::on_entry<_, TEvent>& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state)
    {
    ...
    template <class _, class TEvent, class SM, class TDeps, class TSubs>
    constexpr static bool execute_impl(
        const back::on_exit<_, TEvent>& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state)
    {
        sub_sm<sm_impl<TSM>>::get(&subs).process_event(event, deps, subs);
        transitions<T, Ts...>::execute(event, sm, deps, subs, current_state);
        return true;
...
struct get_mapping<state<S>, state<S>, T, R> : aux::type_list<state_mappings<S, aux::join_t<typename R::types, typename T::types>>> {
};
template <class T, class... Ts>
...
struct conditional_mapping<false, aux::type<T1, aux::inherit<Rs...>>, T2, Ts...> {
    using type = unique_mappings_impl<aux::type<aux::inherit<T1, aux::type<typename T2::element_type>>, aux::inherit<T2, Rs...>>, Ts...>;
};
template <class T1, class T2, class... Rs, class... Ts>
struct unique_mappings_impl<aux::type<T1, aux::inherit<Rs...>>, T2, Ts...>
    : conditional_mapping<
          aux::is_base_of<aux::type<typename T2::element_type>, T1>::value, aux::type<T1, aux::inherit<Rs...>>, T2, Ts...>::type {
};
...
struct mappings<aux::pool<Ts...>>
    : unique_mappings_t<event_mappings<typename Ts::event, aux::inherit<state_mappings<typename Ts::src_state, aux::type_list<Ts>>>>...> {
};
template <class T>
...
struct get_state_mapping {
    using type = aux::conditional_t<
        ...
        decltype(get_state_mapping_impl<_, TUnexpected>((TMappings*)0)), decltype(get_state_mapping_impl<T, TUnexpected>((TMappings*)0))>;
};
template <class S>
...
unique_mappings_t<T1Mappings..., T2Mappings...> get_event_mapping_impl(
    event_mappings<T, aux::inherit<T1Mappings...>>*, event_mappings<_, aux::inherit<T2Mappings...>>*);
template <class T, class TMappings>
struct get_event_mapping_impl_helper
    : aux::conditional<
          ...
          decltype(get_event_mapping_impl<T>((TMappings*)0)), decltype(get_event_mapping_impl<T>((TMappings*)0, (TMappings*)0))>::type {
};
template <class T, class TMappings>
...
template <class T1, class T2, class TMappings>
struct get_event_mapping_impl_helper<on_entry<T1, T2>, TMappings> : decltype(get_event_mapping_impl<on_entry<T1, T2>>((TMappings*)0)) {
};
template <class T1, class T2, class TMappings>
struct get_event_mapping_impl_helper<on_exit<T1, T2>, TMappings> : decltype(get_event_mapping_impl<on_exit<T1, T2>>((TMappings*)0)) {
};
template <class T, class TMappings>
...
struct jump_table {
    ...
    static bool dispatch(
        sm_impl& self, State& current_state, const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TStates...>&)
    {
        using dispatch_table_t = bool (*)(const TEvent&, sm_impl&, TDeps&, TSubs&, State&);
        constexpr static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TStates))] = {&get_state_mapping_t<
            TStates, TMappings, typename sm_impl::has_unexpected_events>::template execute<TEvent, sm_impl, TDeps, TSubs>...};
...
struct branch_stm {
    ...
    template <int N, class TMappings, class sm_impl, class State, class TEvent, class TDeps, class TSubs, class TState, class... TStates>
    constexpr static bool dispatch(
        sm_impl& self, State& current_state, const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TState, TStates...>&)
    {
        return current_state == N ? get_state_mapping_t<TState, TMappings, typename sm_impl::has_unexpected_events>::template execute<
...
struct fold_expr {
    ...
    constexpr static bool dispatch_impl(
        sm_impl& self, State& current_state, aux::index_sequence<Ns...>, const TEvent& event, TDeps& deps, TSubs& subs,
        const aux::type_list<TStates...>&)
    {
        return (
            (current_state == Ns ? get_state_mapping_t<TStates, TMappings, typename sm_impl::has_unexpected_events>::template execute<
                                       TEvent, sm_impl, TDeps, TSubs>(event, self, deps, subs, current_state)
                                 : false) ||
            ...);
    }
    ...
    constexpr static bool dispatch(
        sm_impl& self, State& current_state, const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TStates...>& states)
    {
        return dispatch_impl<TMappings>(self, current_state, aux::make_index_sequence<sizeof...(TStates)>{}, event, deps, subs, states);
    }
};
    #endif
}  // namespace policies
...
template <class SM, class TLogger, class TDeps, class TGuard, class TEvent>
constexpr void log_guard(const aux::type<TLogger>&, TDeps& deps, const aux::zero_wrapper<TGuard>& guard, const TEvent& event, bool result)
{
...
struct thread_safe : aux::pair<thread_safety_policy__, thread_safe<TLock>> {
    ...
    {
        struct lock_guard {
            constexpr explicit lock_guard(TLock& lock)
                : lock_{lock}
            {
                lock_.lock();
            }
...
struct sm_policy {
    ...
    using sm = SM;
    using thread_safety_policy = decltype(get_policy<no_policy, policies::thread_safety_policy__>((aux::inherit<TPolicies...>*)0));
    using defer_queue_policy = decltype(get_policy<no_policy, policies::defer_queue_policy__>((aux::inherit<TPolicies...>*)0));
    using process_queue_policy = decltype(get_policy<no_policy, policies::process_queue_policy__>((aux::inherit<TPolicies...>*)0));
    ...
    using dispatch_policy = decltype(get_policy<default_dispatch_policy, policies::dispatch_policy__>((aux::inherit<TPolicies...>*)0));
    template <class T>
...
struct sm_impl : aux::conditional_t<aux::is_empty<typename TSM::sm>::value, aux::none_type, typename TSM::sm> {
    ...
    using has_history_states = aux::integral_constant<bool, aux::size<initial_states_t>::value != aux::size<history_states_t>::value>;
    ...
    template <
        ...
        __BOOST_SML_REQUIRES(
            !aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value && !aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value &&
    ...
    template <class TEvent, class TDeps, class TSubs, __BOOST_SML_REQUIRES(aux::is_base_of<get_mapped_t<TEvent>, events_ids_t>::value)>
    constexpr bool process_internal_events(const TEvent& event, TDeps& deps, TSubs& subs)
    {
        policies::log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
    #if BOOST_SML_DISABLE_EXCEPTIONS
    ...
    template <class TEvent, class TDeps, class TSubs, __BOOST_SML_REQUIRES(!aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
    ...
    constexpr bool process_internal_generic_event(const TEvent& event, TDeps& deps, TSubs& subs, state_t& current_state)
    {
        policies::log_process_event<sm_t>(aux::type<logger_t>{}, deps, event);
    #if BOOST_SML_DISABLE_EXCEPTIONS
        return process_event_impl<get_event_mapping_t<get_generic_t<TEvent>, mappings>>(event, deps, subs, states_t{}, current_state);
    #else
    ...
    constexpr bool process_event_impl(
        const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TStates...>& states, aux::index_sequence<0>)
    {
    ...
    constexpr bool process_event_impl(
        const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TStates...>& states, aux::index_sequence<Ns...>)
    {
        auto handled = false;
    #if defined(__cpp_fold_expressions)
    ...
    #else
        (void)aux::swallow{
            0, (handled |= dispatch_t::template dispatch<0, TMappings>(*this, current_state_[Ns], event, deps, subs, states), 0)...};
    #endif
        return handled;
    }
    template <class TMappings, class TEvent, class TDeps, class TSubs, class... TStates>
    constexpr bool process_event_impl(
        const TEvent& event, TDeps& deps, TSubs& subs, const aux::type_list<TStates...>& states, state_t& current_state)
    {
    ...
    template <class TMappings, class TEvent, class TDeps, class TSubs>
    bool process_event_except_impl(const TEvent& event, TDeps& deps, TSubs& subs, state_t& current_state, aux::true_type) noexcept
    {
        try {
            return process_event_impl<TMappings>(event, deps, subs, states_t{}, current_state);
        }
    ...
    bool process_defer_events(
        TDeps& deps, TSubs& subs, const bool handled, const aux::type<TDeferQueue>&, const aux::type_list<TEvents...>&)
    {
    ...
    bool process_queued_events(
        TDeps& deps, TSubs& subs, bool& queued_handled, const aux::type<TDeferQueue>&, const aux::type_list<TEvents...>&)
    {
        using dispatch_table_t = bool (sm_impl::*)(TDeps&, TSubs&, const void*);
        const static dispatch_table_t dispatch_table[__BOOST_SML_ZERO_SIZE_ARRAY_CREATE(sizeof...(TEvents))] = {
...
class sm {
    using sm_t = typename TSM::sm;
    using logger_t = typename TSM::logger_policy::type;
    using logger_dep_t = aux::conditional_t<aux::is_same<no_policy, logger_t>::value, aux::type_list<>, aux::type_list<logger_t&>>;
    using transitions_t = decltype(aux::declval<sm_t>().operator()());
...
private:
    ...
    using sub_sms_t =
        aux::apply_t<aux::pool, typename convert_to_sm<TSM, aux::apply_t<aux::unique_t, aux::apply_t<get_sub_sms, states>>>::type>;
    using deps = aux::apply_t<merge_deps, transitions_t>;
...
public:
    ...
    constexpr sm(aux::init, deps_t& deps)
        : deps_{deps}
        , sub_sms_{deps}
    {
        aux::get<sm_impl<TSM>>(sub_sms_).start(deps_, sub_sms_);
    ...
    template <
        class T = aux::identity<sm_t>, class TState,
    ...
    #else
        (void)aux::swallow{0, (sm.current_state_[region++] = aux::get_id<state_t, typename TStates::type>((states_ids_t*)0), 0)...};
    #endif
    }
    template <class T>
...
private:
    ...
    #else
        (void)aux::swallow{
            0, (call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(aux::get_by_id<Ns>(&a), event, sm, deps, subs), 0)...};
    #endif
    }
    aux::tuple<Ts...> a;
};
...
private:
    ...
    constexpr auto for_all(const aux::index_sequence<Ns...>&, const TEvent& event, TSM& sm, TDeps& deps, TSubs& subs)
    {
        ...
        (void)aux::swallow{
            0,
            (result =
                 result && call<TEvent, args_t<Ts, TEvent>, typename TSM::logger_t>::execute(aux::get_by_id<Ns>(&g), event, sm, deps, subs),
             0)...};
        return result;
    #endif
    }
...
template <class T>
auto transitional_impl(T&& t) -> aux::always<
    typename T::dst_state, typename T::src_state, typename T::event, typename T::deps, decltype(T::initial), decltype(T::history)>;
template <class T>
...
struct transition<state<S1>, transition<state<S2>, G, A>> : transition<state<S1>, state<S2>, front::event<back::anonymous>, G, A> {
...
struct transition<state<S1>, transition<front::event<E>, G, A>> : transition<state<internal>, state<S1>, front::event<E>, G, A> {
...
struct transition<state<S1>, transition<state<S2>, front::event<E>>> : transition<state<S1>, state<S2>, front::event<E>, always, none> {
...
struct transition<state<S1>, transition_sg<state<S2>, G>> : transition<state<S1>, state<S2>, front::event<back::anonymous>, G, none> {
...
struct transition<state<S1>, transition_sa<state<S2>, A>> : transition<state<S1>, state<S2>, front::event<back::anonymous>, always, A> {
...
struct transition<state<S2>, transition_eg<front::event<E>, G>> : transition<state<internal>, state<S2>, front::event<E>, G, none> {
...
struct transition<state<S2>, transition_ea<front::event<E>, A>> : transition<state<internal>, state<S2>, front::event<E>, always, A> {
...
constexpr void update_current_state(
    SM&, TDeps& deps, TSubs&, typename SM::state_t& current_state, const typename SM::state_t& new_state, const TSrcState&,
    const TDstState&)
{
    back::policies::log_state_change<typename SM::sm_t>(
        aux::type<typename SM::logger_t>{}, deps, aux::string<typename TSrcState::type>{}, aux::string<typename TDstState::type>{});
    current_state = new_state;
}
template <class SM, class TDeps, class TSubs, class TSrcState, class T>
constexpr void update_current_state(
    SM&, TDeps& deps, TSubs& subs, typename SM::state_t& current_state, const typename SM::state_t& new_state, const TSrcState&,
    const state<back::sm<T>>&)
{
    back::policies::log_state_change<typename SM::sm_t>(
        aux::type<typename SM::logger_t>{}, deps, aux::string<typename TSrcState::type>{}, aux::string<T>{});
    current_state = new_state;
    update_composite_states<back::sm_impl<T>>(
        subs, typename back::sm_impl<T>::has_history_states{}, typename back::sm_impl<T>::history_states_t{});
}
template <class S1, class S2, class E, class G, class A>
    ...
    constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state, aux::true_type)
    {
        if (call<TEvent, args_t<G, TEvent>, typename SM::logger_t>::execute(g, event, sm, deps, subs)) {
            ...
            update_current_state(
                sm, deps, subs, current_state, aux::get_id<typename SM::state_t, dst_state>((typename SM::states_ids_t*)0),
...
struct transition<state<S1>, state<S2>, front::event<E>, always, A> {
    ...
    constexpr transition(const always&, const A& a)
        : a(a)
    {
    }
    template <class TEvent, class SM, class TDeps, class TSubs>
    constexpr bool execute(const TEvent& event, SM& sm, TDeps& deps, TSubs& subs, typename SM::state_t& current_state, aux::true_type)
    {
        sm.process_internal_event(back::on_exit<back::_, TEvent>{event}, deps, subs, current_state);
        update_current_state(
            sm, deps, subs, current_state, aux::get_id<typename SM::state_t, dst_state>((typename SM::states_ids_t*)0), state<src_state>{},
            state<dst_state>{});
        call<TEvent, args_t<A, TEvent>, typename SM::logger_t>::execute(a, event, sm, deps, subs);
        sm.process_internal_event(back::on_entry<back::_, TEvent>{event}, deps, subs, current_state);
        return true;
...
struct transition<state<S1>, state<S2>, front::event<E>, G, none> {
    ...
    constexpr transition(const G& g, const none&)
        : g(g)
    {
    }
    template <class TEvent, class SM, class TDeps, class TSubs>
...
template <
    class... Ts,
    __BOOST_SML_REQUIRES(
        aux::is_same<aux::bool_list<aux::always<Ts>::value...>, aux::bool_list<concepts::transitional<Ts>::value...>>::value)>
